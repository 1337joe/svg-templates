...
function updateTemplateResults(event) {
  ...
  if (topDiameter == bottomDiameter) {
    ...
    svg.setAttribute("viewBox", `0 0 ${templateWidth} ${templateHeight}`);
    ...
    if (hasLogo) {
      ...
      // TODO notch top and bottom on centerline to help with vertical alignment
      const logoMaskHeight = templateHeight - logoCenterHeight + logoHeight / 2 + logoMaskRadius;
      const logoMask = document.createElementNS(nsSvg, "rect");
      logoMask.setAttribute("x", (templateWidth - logoWidth) / 2 - logoMaskRadius);
      logoMask.setAttribute("y", 0);
      logoMask.setAttribute("width", logoWidth + 2 * logoMaskRadius);
      logoMask.setAttribute("height", logoMaskHeight);
  ...
  if (flip) {
    ...
    maxY = fullCone.outerRadius;
  } else {
    minY = -fullCone.outerRadius;
  ...
  if (hasLogo) {
    ...
    updateGraphicTableResults(units, toMm, 
